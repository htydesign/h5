"undefined" == typeof jwplayer && (jwplayer = function(d) {
		...
		b.filterPlaylist = function(a, c) {
			...
			if (c &&
				...
				for (g = 0; g < a.length; g++)
					if (j = b.extend({}, a[g]), j.sources = b.filterSources(j.sources, !0), 0 < j.sources.length) {
		...
		b.filterSources = function(a, c) {
			...
			if (a) {
				...
				for (var f = 0; f < a.length; f++) {
					...
					c ? d.embed.flashCanPlay(n, h) && (e || (e = h), h == e && g.push(b.extend({}, a[f]))) : b.canPlayHTML5(h) && (e ||
		...
		b.seconds = function(a) {
			...
			var b = a.split(":"),
				c = 0;
			"s" == a.substr(-1) ? c = Number(a.substr(0, a.length - 1)) : "m" == a.substr(-1) ? c = 60 * Number(a.substr(0, a.length - 1)) : "h" == a.substr(-1) ? c = 3600 * Number(a.substr(0, a.length - 1)) : 1 < b.length ? (c = Number(b[b.length - 1]), c += 60 * Number(b[b.length -
				2]), 3 == b.length && (c += 3600 * Number(b[b.length - 3]))) : c = Number(a);
			return c
		};
		...
		d.xmlAttribute = function(a, d) {
			for (var f = 0; f < a.attributes.length; f++)
				if (a.attributes[f].name && a.attributes[f].name.toLowerCase() ==
					d.toLowerCase()) return a.attributes[f].value.toString();
			return ""
		};
		...
		d.key = function(a) {
			...
			try {
				...
				var e = a.split("/");
				(k = e[0]) ? /^(free|pro|premium|ads)$/i.test(k) ? (f = e[1], e[2] && 0 < parseInt(e[2]) &&
		...
		var k = {
				...
				encode: function(a, e) {
					...
					if (0 < l)
						for (; 3 > l++;) p += "\x3d", g += "\x00";
					for (l = 0; l < g.length; l += 3) j = g.charCodeAt(l), b = g.charCodeAt(l +
						1), c = g.charCodeAt(l + 2), d = j << 16 | b << 8 | c, j = d >> 18 & 63, b = d >> 12 & 63, c = d >> 6 & 63, d &= 63, m[l / 3] = q.charAt(j) + q.charAt(b) + q.charAt(c) + q.charAt(d);
					m = m.join("");
				...
				decode: function(a, e) {
					...
					var j, b, c, d, m, p = [],
						l, g = k.code;
					l = e ? f.decode(a) : a;
					for (var q = 0; q < l.length; q += 4) j = g.indexOf(l.charAt(q)), b = g.indexOf(l.charAt(q + 1)), d = g.indexOf(l.charAt(q + 2)), m = g.indexOf(l.charAt(q + 3)), c = j << 18 | b << 12 | d << 6 | m, j = c >>> 16 & 255, b = c >>> 8 & 255, c &= 255, p[q / 4] = String.fromCharCode(j, b,
						c), 64 == m && (p[q / 4] = String.fromCharCode(j, b)), 64 == d && (p[q / 4] = String.fromCharCode(j));
					d = p.join("");
		...
		d.eventdispatcher = function(d, f) {
			...
			this.addGlobalListener = function(f, b) {
				try {
					"string" == a.typeOf(f) && (f = (new Function("return " + f))()), e.push({
						listener: f,
						count: b
					})
				} catch (c) {
	...
	function(d) {
		...
		(a.jwparser = function() {}).parseEntry = function(k, f) {
			...
			if (h.length) {
				f.sources = [];
				for (b = 0; b < h.length; b++) 0 < h[b].file.length && (h[b]["default"] = "true" == h[b]["default"] ?
		...
		(d.embed.flash = function(e, j, b, c, n) {
			...
			this.embed = function() {
				...
				a.isIE() ? (h = '\x3cobject classid\x3d"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" " width\x3d"100%" height\x3d"100%"id\x3d"' + e.id + '" name\x3d"' + e.id + '" tabindex\x3d0""\x3e', h += '\x3cparam name\x3d"movie" value\x3d"' + j.src + '"\x3e', h += '\x3cparam name\x3d"allowfullscreen" value\x3d"true"\x3e\x3cparam name\x3d"allowscriptaccess" value\x3d"always"\x3e',
					...
					e.id), h.setAttribute("tabindex", 0), m(h, "allowfullscreen", "true"), m(h, "allowscriptaccess", "always"), m(h, "seamlesstabbing", "true"), m(h, "wmode", d), e.parentNode.replaceChild(h, e), d = h);
	...
	function(d) {
		var a = d.embed,
			...
			f = k.extend(function(f) {
				...
				c.key = j;
				b.analytics && (b.analytics.client &&
					b.analytics.client.match(".js$|.swf$")) && (p = b.analytics.client);
				delete c.analytics;
				...
				switch (r) {
					...
					case "pro":
						b.sharing && (b.sharing.client && b.sharing.client.match(".js$|.swf$") &&
				...
				function l() {
					if (u) {
						for (var a = arguments[0],
								b = [], c = 1; c < arguments.length; c++) b.push(arguments[c]);
			...
			function n(b, d) {
				g[b] || (g[b] = [], c(a.JWPLAYER_PLAYER_STATE, function(a) {
					...
					if (c == b) {
						...
						if (e)
							for (var f =
									0; f < e.length; f++) "function" == typeof e[f] && e[f].call(this, {
								oldstate: d,
								newstate: c,
								type: a.type
							})
