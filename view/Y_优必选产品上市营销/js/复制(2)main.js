...
function onTouchSlide() {
	...
	var oContainerHeight = oContainer.offsetHeight - window.innerHeight;
	...
	function onTouchEnd(e) {
		...
		if(isRun) {
			...
			runY = moveY / moveLength * 400;

			translateY += -runY;

			if(translateY >= 0) {
				translateY = 0;
			} else if(translateY <= -oContainerHeight) {
				translateY = -oContainerHeight
			}
			t = moveY / moveLength * runTime;
			oContainer.style.transform = "translate3d(0px," + translateY + "px,0px)";
			oContainer.style.webkitTransform = "translate3d(0px," + translateY + "px,0px)";
			oContainer.style.transitionDuration = t + "s";
			oContainer.style.webkitTransitionDuration = t + "s";

			onScrollTop(translateY);

			moveY = 0;

			oContainer.removeEventListener('touchmove', onTouchMove, false);
			oContainer.removeEventListener('touchend', onTouchEnd, false);
