...
function createScene() {
	...
	function createWall(num,y,container){
		...
		//右边墙
		createPlane(wallSize_1.w, wallSize_1.h, data[num].right, container, -wallSize_2.w / 2*iosdir, 0, 0, 0, Math.PI / 180 * 90*iosdir, 0, "rightWall");		
		//左边墙
		createPlane(wallSize_1.w, wallSize_1.h, data[num].left, container, wallSize_2.w / 2*iosdir, 0, 0, 0, -Math.PI / 180 * 90*iosdir, 0, "leftWall");	
		//正前方墙
		createPlane(wallSize_2.w, wallSize_2.h, data[num].front, container, 0, 0, wallSize_1.w / 2*iosdir, 0, Math.PI*iosdir, 0, "frontWall");
		//后面墙
		createPlane(wallSize_2.w, wallSize_2.h, data[num].back, container, 0, 0, -wallSize_1.w / 2*iosdir, 0, 2*Math.PI*iosdir, 0, "backWall");
		//地板
...
function slideBlock(){	
	var image_height=2691;
	var startY=0;
	var moveY=0;
	var endY=0;
	var sx=0;
	var h=window.innerHeight-image_height;
	$(window).on('resize',function(){
		h=window.innerHeight-image_height;
	});
	var h2=(h+image_height-100);
	var l=0;
	var disY=0;
	...
	function onTouchMove(e){
		...
		posterBox.style.transform = 'translate3d(0px,' + disY + 'px,0px)';
		posterBox.style.webkitTransform = 'translate3d(0px,' + disY + 'px,0px)';
		
		sx=h2*disY/h;
		oBar.style.transform = 'translate3d(0px,' + sx + 'px,0px)';
		oBar.style.webkitTransform = 'translate3d(0px,' + sx + 'px,0px)';
	}
...
function createShowSlideImages(x) {
	...
	for(var i = 0; i < aImages[n].images.length; i++) {
		...
		oImg.src = basicURL + aImages[n].theme + "/" + aImages[n].images[i];
		//oP.innerHTML = aImages[n].txt[i];
		oLi.style.width=lw+'px';
		oImg.style.width=lw+'px';
		
		var userImage = new Image();
		userImage.src = basicURL + aImages[n].theme + "/" + aImages[n].images[i];
